...
private class FolderSelectionListener(
  ...
  override fun valueChanged(e: TreeSelectionEvent) {
    ...
    val parent = getRealFile8(node?.userObject as? File)
    if (node?.isLeaf == true && parent != null && parent.isDirectory) {
  ...
  private fun getRealFile8(file: File?) = runCatching {
    val sf = ShellFolder.getShellFolder(file)
    if (sf.isLink) sf.linkLocation else file
  }.getOrNull()

  // private fun getRealFile9(file: File?) = runCatching {
