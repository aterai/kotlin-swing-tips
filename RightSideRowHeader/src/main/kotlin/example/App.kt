...
private class RightFixedScrollPaneLayout : ScrollPaneLayout() {
  ...
  override fun layoutContainer(parent: Container) {
    ...
    val sv = if (!isEmpty && view is Scrollable) {
      viewTracksViewportWidth = view.scrollableTracksViewportWidth
      viewTracksViewportHeight = view.scrollableTracksViewportHeight
      view
    } else {
      null
    }
  ...
  private fun adjustForVsb(
    ...
    if (wantsVsb) {
      val vsbWidth = maxOf(0, minOf(vsb.preferredSize.width, available.width))
  ...
  private fun adjustForHsb(wantsHsb: Boolean, available: Rectangle, hsbR: Rectangle, vpbInsets: Insets) {
    ...
    if (wantsHsb) {
      val hsbHeight = maxOf(0, minOf(available.height, hsb.preferredSize.height))
