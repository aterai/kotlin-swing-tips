...
fun makeUI(): Component {
  ...
  area.append("%s: %s%n".format(key, UIManager.getString(key)))

  val textField = JTextField("Title Progress...")
  val check = JCheckBox("Default")
  check.addActionListener { textField.setEnabled(!check.isSelected) }

  val monitor = ProgressMonitor(null, "message", "note", 0, 100)

  val runButton = JButton("run")
  runButton.addActionListener {
    runButton.isEnabled = false
    val title = if (check.isSelected) null else textField.getText()
